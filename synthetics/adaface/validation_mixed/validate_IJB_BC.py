...
def infer_images(model, img_root, landmark_list_path, batch_size, use_flip_test, fusion_method, gpu_id):
    ...
    for img_index, each_line in enumerate(files):
        ...
        img_path = os.path.join(img_root, name_lmk_score[0][:2], name_lmk_score[0])
...
def identification(data_root, dataset_name, img_input_feats, save_path):
    ...
    meta_dir = os.path.join(data_root, 'meta', dataset_name+"_meta")
...
def verification(data_root, dataset_name, img_input_feats, save_path):
    templates, medias = eval_helper_verification.read_template_media_list(
        os.path.join(data_root, 'meta', '%s_meta' % dataset_name, '%s_face_tid_mid.txt' % dataset_name.lower()))
    p1, p2, label = eval_helper_verification.read_template_pair_list(
        os.path.join(data_root, 'meta', '%s_meta' % dataset_name,
...
def load_pretrained_model(ckpt_path, arch='ir_50'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', default='../pretrained/adaface_ir50_ms1mv2.ckpt')
    ...
    save_path = '{}/{}/{}'.format(args.output_dir, args.dataset_name, args.arch)
    ...
    img_root = os.path.join(args.data_root, './ijb/%s/loose_crop' % dataset_name)
    landmark_list_path = os.path.join(args.data_root, './ijb/%s/meta/%s_name_5pts_score.txt' % (dataset_name, dataset_name.lower()))
